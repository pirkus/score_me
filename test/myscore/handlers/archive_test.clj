...
(deftest archive-scorecard-handler-test
  (let [db (fresh-db)
    ...
    (testing "Invalid ID format rejection"
      (let [request {:path-params {:id "!"}} ;; Use a character that's not valid in base64 or ObjectId
            response (handler request)]
  ...
  (let [db (fresh-db)
    ...
    (testing "Only non-archived scorecards are returned by default"
      (let [request {:identity {:email email}}
        ...
        ;; Check encoded ID is returned
        (is (contains? (first body) :id))
        (is (= (str id1) (:id (first body))))))
...
(deftest archive-encoded-id-test
  (let [db (fresh-db)
        archive-handler (system/archive-scorecard-handler db)
    ...
    ;; Verify the scorecard is in the DB
    (let [initial-doc (mc/find-map-by-id db "scorecards" scorecard-id)]
      (is (not (nil? initial-doc)))
      (is (= "Encoded ID Test" (:configName initial-doc)))
      (is (nil? (:archived initial-doc)))
    
      ;; Now archive it
      (let [archive-request {:path-params {:id (str scorecard-id)}}
            archive-response (archive-handler archive-request)]
        (is (= 200 (:status archive-response)))
        
        ;; Verify the response
        (let [body (json/parse-string (:body archive-response) true)]
          (is (= "archived" (:result body)))
          (is (= (str scorecard-id) (:id body))))
        
        ;; Check it was updated in the database directly
        (let [updated-doc (mc/find-map-by-id db "scorecards" scorecard-id)]
          (is (= true (:archived updated-doc))))))))